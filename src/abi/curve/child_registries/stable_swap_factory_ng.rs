...
pub mod functions {
    ...
    pub struct AddBasePool {
        ...
        pub u_base_lp_token: Vec<u8>,
        pub u_asset_types: Vec<substreams::scalar::BigInt>,
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(
                &[
                    ...
                    ethabi::Token::Address(
                        ethabi::Address::from_slice(&self.u_base_lp_token),
                    ),
                    {
                        let v = self
                            .u_asset_types
    ...
    pub struct DeployMetapool {
        ...
        pub u_offpeg_fee_multiplier: substreams::scalar::BigInt,
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(
                &[
                    ...
                    ethabi::Token::Uint(
                        ethabi::Uint::from_big_endian(
                            match self.u_offpeg_fee_multiplier.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                    panic!("negative numbers are not supported")
                    ...
                    ethabi::Token::FixedBytes(self.u_method_id.as_ref().to_vec()),
                    ethabi::Token::Address(ethabi::Address::from_slice(&self.u_oracle)),
                ],
            );
            let mut encoded = Vec::with_capacity(4 + data.len());
            encoded.extend(Self::METHOD_ID);
    ...
    pub struct DeployPlainPool {
        ...
        pub u_implementation_idx: substreams::scalar::BigInt,
        pub u_asset_types: Vec<substreams::scalar::BigInt>,
        pub u_method_ids: Vec<[u8; 4usize]>,
        pub u_oracles: Vec<Vec<u8>>,
    }
